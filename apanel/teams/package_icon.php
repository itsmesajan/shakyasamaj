...
<div class="col-md-3" id="previewRoomsimage<?php echo $deleteid;?>">
    <div class="infobox info-bg">
        <img src="<?php echo IMAGE_PATH.'package/icon/thumbnails/'.$val;?>"  style="width:100%"/>
        ...
        <input type="hidden" name="imageArrayname5[]" value="<?php echo $val;?>" class="validate[required,length[0,250]]" />        
    </div> 
</div>
...
//uplodify
  if(isset($_SESSION['imageNameArr'])){
   if(count($_SESSION['imageNameArr'])>0){
      ...
      { 
         @unlink(IMAGE_PATH.'package/banner/thumbnails/'.$val);
         @unlink(IMAGE_PATH.'package/banner/'.$val);
      }  
