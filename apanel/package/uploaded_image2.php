...
<div class="col-md-3" id="previewRoomsimage<?php echo $deleteid;?>">
    <div class="infobox info-bg">
        <img src="<?php echo IMAGE_PATH.'subpackage/related/thumbnails/'.$val;?>"  style="width:100%"/>
        ...
        <input type="hidden" name="imageArrayname3" value="<?php echo $val;?>" class="validate[length[0,250]]" />        
    </div> 
</div>
</div>
...
//uplodify
  if(isset($_SESSION['imageNameArr'])){
   if(count($_SESSION['imageNameArr'])>0){
      ...
      { 
         @unlink(IMAGE_PATH.'subpackage/related/thumbnails/'.$val);
      	 @unlink(IMAGE_PATH.'subpackage/related/'.$val);
      }	 
