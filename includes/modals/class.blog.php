...
class Blog extends DatabaseObject
{
    ...
    protected static $db_fields = array('id', 'slug', 'title', 'author', 'brief', 'content', 'status', 'meta_keywords', 'meta_description', 'blog_date','linksrc','linktype', 'archive_date', 'sortorder', 'type', 'viewcount', 'added_date', 'image', 'source','b_image');
    ...
    public static function checkDuplislug($slug='')
	{
		global $db;
		$query = $db->query("SELECT slug FROM ".self::$table_name." WHERE slug=\"$slug\" LIMIT 1");
		$result= $db->num_rows($query);
		if($result>0) {return true;}
	}
    ...
    //get otherblogs
    public static function get_relatedblog($notid='', $limit='')
	{
		global $db;
		$cond2 = !empty($notid)?' AND id<>'.$notid:'';
		$cond3 = !empty($limit)?' LIMIT '.$limit:'';
		$sql = "SELECT * FROM ".self::$table_name." WHERE status=1 $cond2 ORDER BY sortorder DESC ".$cond3;
		return self::find_by_sql($sql);
	}
}
