...
//			$Package->country   	= $_REQUEST['country'];
...
//			$Package->country  = $_REQUEST['country'];
		...
		case "addSubpackage":
			...
			$record->event_type 			= !empty($_REQUEST['event_type']) ? $_REQUEST['event_type'] : '';
			$record->detail 		= !empty($_REQUEST['detail'])?$_REQUEST['detail']:'';
//			$record->linksrc 	= $_REQUEST['linksrc'];
			// $record->dining_logo			= !empty($_REQUEST['imageArrayname3'])?$_REQUEST['imageArrayname3']:'';
			...
			$record->projectstatus			= $_REQUEST['projectstatus'];
...
//				$record->linksrc 	= $_REQUEST['linksrc'];
        ...
        case "togglehome":
            $id = $_REQUEST['id'];
            $record = Subpackage::find_by_id($id);
            $record->number_room = ($record->number_room == 1) ? 0 : 1 ;
            $record->save();
            echo "";
        break;
