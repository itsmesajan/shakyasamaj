...
function getSingleUnserializedImageSrc( $image,$path = ''){
    ...
    if($image != 'a:0:{}'){
        $unserializedImage = unserialize($image)[0];
        $file_path = SITE_ROOT . 'images/'. $path .'/' . $unserializedImage;
        if(file_exists($file_path)){
            $imgSrc = IMAGE_PATH . $path .'/'. $unserializedImage;
...
if(defined('HOME_PAGE')){
    $eventSliderItem = '';
    $eventStatus = Array(0 => "Completed",1 => "Ongoing", 2 => "Upcoming");
    $subRec = Subpackage::getPackage_limit(0);
    foreach($subRec as $subRow){
        $status ='';
        foreach($eventStatus as $key => $value){
            if($key == $subRow->projectstatus){
                $status = $value;
            }
        }
        $imgSrc = getSingleUnserializedImageSrc($subRow->image,'subpackage');
        if($subRow->number_room == 1){
            $eventSliderItem .= '
                        ...
                        <div class="events_item">
                            <div class="event_inner">
                                ...
                                <div class="bottom_text">
                                    ...
                                    <p>'. $subRow->event_type .'</p>
                                    <h3>'. $subRow->title .'</h3>
                                    <a href="'. BASE_URL .''. $subRow->slug .'"><i class="fa fa-arrow-right"></i></a>
                                </div>
                                <!--bottom_text-->
                                <div class="mask  mask_primary"></div>
                            </div>
                            <!--event_top-->
                        </div>
                        <!-- /.events_item -->
    ...
    $mainContent = '
            <section id="events" class="events_slide content gray-bg">
                <div class="container">
                    <div class="row mb-60">
                        <div class="col-lg-8 offset-lg-2 text-center">
                            <p class="rdn_subtext" data-cue="slideInUp">Come and join today</p>
                            <h2 class="rdn_title mb-0" data-cue="slideInUp" data-delay="400">
                                Here is our newest upcoming event in your town.
                            </h2>
                        </div>
                ...
                <!-- /.container -->
                <div class="circle circle_left" data-cue="slideInLeft"></div>
            </section>
    ';
    $jVars['module:home-events'] = $mainContent;
}


if(defined('PACKAGE_PAGE')){
    $mainContent = $items = $banner = '';
    $subRec = Subpackage::get_relatedsub_by(0);
    foreach($subRec as $subRow){
        if($subRow->projectstatus == $_REQUEST['project_status']){
            $imgSrc = getSingleUnserializedImageSrc($subRow->image,'subpackage');
            
            $items .= '
                <div class="col-lg-4 align-self-center mb-30 event_outer col-md-6">
                    <div class="events_item">
                        <div class="event_top">
                            <div class="top_icon">
                        ...
                        <!--event_top-->
                        <div class="bottom_text">
                            <h3><a href="'. BASE_URL .''. $subRow->slug .'">'. $subRow->title .'</a></h3>
                        </div>
                        <!--bottom_text-->
                    </div>
                    <!-- /.events_item -->
    ...
    $states = [0 => 'Completed', 1 => 'Ongoing', 2 => 'Upcoming'];
    $title = '';
    foreach($states as $key => $state){
        if( $key == $_REQUEST['project_status']){
            $title = $state;
            break;
        }
    ...
    if(!empty($siteRegulars->event_upload)){
        $file_path = SITE_ROOT . 'images/preference/events/' . $siteRegulars->event_upload;
        if(file_exists($file_path)){
            $imgSrc = IMAGE_PATH . 'preference/events/' . $siteRegulars->event_upload;
        }else if(file_exists($defaultImg)){
            $imgSrc = IMAGE_PATH . 'preference/other/' . $siteRegulars->other_upload;
        }else{
            $imgSrc = BASE_URL . 'template/web/img/background/about.jpg';
    ...
    $jVars['module:package-events'] = $mainContent;
}
if(defined('SUBPACKAGE_PAGE') && isset($_REQUEST['slug'])){
    $mainContent = '';
    $subPkgDetail = Subpackage::find_by_slug($_REQUEST['slug']);
    $subPkgImages = SubPackageImage::getImagelist_by($subPkgDetail->id);

    $subPkgCarou = '';
    if(!empty($subPkgImages)){
        foreach($subPkgImages as $subImg){
            $imgSrc = '';
            $file_path = SITE_ROOT . 'images/package/galleryimages/' . $subImg->image;
            if(file_exists($file_path)){
                $imgSrc = IMAGE_PATH . 'package/galleryimages/' . $subImg->image;

                $subPkgCarou .= '
                    <div class="swiper-slide">
                        <img src="'. $imgSrc .'" alt="'. $subImg->title .'">
                    </div>
                ';
            }
            ...
            <div class="container">
                <div class="row align-content-center mb-100 mt-100">
                    <div class="col-lg-6 align-self-center mb-30 mb-lg-0">
                        <h2 class="rdn_title mb-0" data-cue="slideInUp" data-delay="200">'. $subPkgDetail->title .'</h2>
                    </div>
                    <!-- col-lg-6 -->

                    <div class="col-lg-6 align-self-end">
                        <p class="rdn_boldtext">'. $subPkgDetail->detail .'</p>
                    </div>
                    <!-- col-lg-6 -->
                </div>
                <!--row-->
                ...
                <div class="row">
                    <div class="col-lg-3">
                        <ul class="single_event_list gray-bg list-unstyled">
                            <li><i class="ph-map-pin-bold"></i> '. $subPkgDetail->seats .'</li>
                            <li><i class="ph-calendar-blank-bold"></i> '. $subPkgDetail->twop_price .'</li>
                            <li><i class="ph-clock-afternoon-bold"></i> '. $subPkgDetail->currency .'</li>
                            <li><i class="ph-user-circle-bold"></i> '. $subPkgDetail->onep_price .'</li>
                        </ul>
                    </div>
                    <!-- col-lg-3 -->
                    <div class="col-lg-8 offset-lg-1">
                        '. $subPkgDetail->content .'
                        <!-- /.row -->
                    </div>
                    <!-- col-lg-8 -->
                </div>
    ...
    $jVars['module:subpkg-content'] = $mainContent;
}