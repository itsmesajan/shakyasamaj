<?php 
	...
	{			
		...
		case "add":	
			...
			!(empty($_REQUEST['imageArrayname1'])) ? ($record->b_image	= $_REQUEST['imageArrayname1']): ($record->b_image	= '');
			...
			$checkDuplicateTitle = Blog::checkDupliTitle($record->title);
			$checkDuplicateSlug = Blog::checkDuplislug($record->slug);
			if($checkDuplicateTitle || $checkDuplicateSlug){
				echo json_encode(["action"=>"warning","message"=>"Title or slug already exists!"]);
				exit;
			}
