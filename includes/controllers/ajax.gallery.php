<?php 
	...
	{			
		...
		case "addSubGalleryImage":
			...
			$content  	= !empty($_REQUEST['imgContent'])?$_REQUEST['imgContent']:'';
			$galleryid  = $_REQUEST['galleryid'];
			if(!empty($imageName)):
			foreach($imageName as $key=>$val):
				$FimageName		= $imageName[$key];
				$Ftitle	        = $title[$key];																	
				$FContent	        = $content[$key];																	
				//Save Record
				if(!empty($FimageName)):
        ...
        case "editSubGalleryImageText":
            ...
            if(!empty($GalleryImage)){
				$GalleryImage->title = !empty($_REQUEST['title']) ? $_REQUEST['title'] : '';
                $GalleryImage->content = !empty($_REQUEST['content']) ? $_REQUEST['content'] : '';
                $db->begin();
