    ...
    foreach ($socialRec as $socialRow) {
        if($socialRow->type == 1){
            ...
            if(!empty($icon)){
                ...
                $disicon='<a href="' . $socialRow->linksrc . '"><i class="' . $socialRow->icon . '"></i></a>';
            }
...
if(!empty($socialRec)){
    foreach($socialRec as $socialRow){
        if($socialRow->type == 2){
            $img = $socialRow->image;
            if(!empty($img)){
                $supports .= '<div class="swiper-slide align-self-center"><a href="#"><img src="'. IMAGE_PATH.'social/' . $socialRow->image .'" alt="client"></a></div>';
            }else if(!empty($socialRow->icon)){
                $supports .= '<div class="swiper-slide align-self-center"><a href="#"><i class="'. $socialRow->icon .'"></i></a></div>';
            }
        }
    }
}
...
$jVars['module:support-brand'] = $supports;
