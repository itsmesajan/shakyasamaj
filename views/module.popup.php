...
if(!empty($tstRec)){
    ...
    $count = 1;
    $auto = (count($tstRec) == 1) ? 'autoplay=1&mute=1' : '';
    foreach ($tstRec as $tstRow) {
        //if(!empty($tstRow->source){
        $active = ($count == 1) ? 'active' : '';
        $parts = explode('.',$tstRow->source);
        ...
        <div class="modal fade" id="exampleModal2" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <button type="button" class="btn-close" id="popupClose2" data-bs-dismiss="modal" aria-label="Close"><i class="fa fa-times"></i></button>
    ...
    ';
}else if(!empty($popRec)){
    $imgList = $imgItem = $controls ='';
    foreach($popRec as $popRow){
        if($popRow->image != 'a:0:{}'){
            $unserializedImage = unserialize($popRow->image);
            foreach($unserializedImage as $img){
                $imgItem .= '
                    <div class="swiper-slide">
                        <img src="'. IMAGE_PATH .'popup/'. $img .'" alt="'. $popRow->title .'" class="popup-image" /> 
                    </div>
                ';
            }
        }
    }
    if(sizeof(unserialize($popRow->image)) > 1) {
        $controls .= '
            <div class="swiper-button-prev"></div>
            <div class="swiper-button-next"></div>
        ';
        ...
        <div class="modal fade" id="exampleModal2" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <button type="button" class="btn-close" id="popupClose2" data-bs-dismiss="modal" aria-label="Close"></button>
                        <div class="swiper">
                            <div class="swiper-wrapper">
                                '. $imgItem .'
                            </div>
