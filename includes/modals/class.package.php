...
class Package extends DatabaseObject
{
    ...
    protected static $db_fields = array('id', 'slug', 'image', 'header_image', 'banner_image', 'title', 'sub_title', 'status', 'sortorder', 'detail', 'content', 'country', 'meta_title', 'meta_keywords', 'meta_description', 'type', 'added_date', 'modified_date');
    ...
    public static function get_internal_link($Lsel = '', $LType = 0)
    {
        ...
        if ($pages):
            ...
            foreach ($pages as $pageRow):
                ...
                $overview = ($pageRow->id != 1) ? 'overview' : '';
//                $result .= '<option value="' . $pakgLink . '/' . $pageRow->slug . '/'.$overview.'" ' . $sel . '>&nbsp;&nbsp;' . $pageRow->title . '</option>';
