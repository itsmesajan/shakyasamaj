...
class Teams extends DatabaseObject {
	...
	protected static $table_name = "tbl_teams";
	protected static $db_fields = array('id', 'slug', 'image', 'header_image', 'banner_image', 'title', 'sub_title','pac_icon', 'status', 'sortorder', 'detail', 'content', 'meta_title', 'meta_keywords', 'meta_description', 'type', 'added_date');
	...
	public static function get_accommodationId()
	{
		...
		public static function get_facilityId()
	{
		global $db;
		$sql="SELECT id FROM ".self::$table_name." WHERE status='1' AND type='0' ORDER BY sortorder DESC LIMIT 1 ";
		$result = $db->query($sql);
		$return = $db->fetch_array($result);
		return ($return) ? $return['id'] : '0';
	}
	...
	// view doctor Front.
	static function getDoctor($limit=''){
		global $db;
	...
	{
		global $db;		
		$sql = "SELECT * type FROM ".self::$table_name." WHERE status='1' ORDER BY sortorder ASC";
		$pages = self::find_by_sql($sql);		
		$linkpageDis = ($Lsel==1)?'hide':'';
		
		$result='';		
		if($pages):
		$result.='<optgroup label="teams">';
			foreach($pages as $pageRow):
				$chkChild  = SubTeams::getTotalSub($pageRow->type);
				$pakgLink  = !empty($chkChild)?'teams':'teams';

				$sel = ($Lsel==($pakgLink.'/'.$pageRow->slug)) ?'selected':'';
				$result.='<option value="'.$pakgLink.'/'.$pageRow->slug.'" '.$sel.'>&nbsp;&nbsp;'.$pageRow->title.'</option>';

				// Sub doctor list
				$subRec = SubTeams::getDoctor_limit($pageRow->id);
				if($subRec){
					foreach($subRec as $Nrow){
						$sel = ($Lsel==("subteams/".$Nrow->slug)) ?'selected':'';
						$result.='<option value="subteams/'.$Nrow->slug.'" '.$sel.'>&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;'.$Nrow->title.'</option>';
					}
				}
	...
	// view doctor of the nos provided.
	static function getDoctorList($total=5, $offset=0){
		global $db;
	...
	// get the doctor name from it's id
	static function getDoctorName($id=0){
		global $db;
		$result = $db->query("SELECT title FROM tbl_teams WHERE id='{$id}'");
		$return = $db->fetch_array($result);
		return ($return) ? $return['title'] : '' ;
	}
	// get the doctor name from it's id
	static function getSubDoctorName($id=0){
		global $db;
		$result = $db->query("SELECT title FROM tbl_teams_sub WHERE id='{$id}'");
		$return = $db->fetch_array($result);
		return ($return) ? $return['title'] : '' ;
	}
