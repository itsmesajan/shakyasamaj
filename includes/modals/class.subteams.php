<?php
class SubTeams extends DatabaseObject {

	protected static $table_name = "tbl_teams_sub";
	protected static $db_fields = array('id', 'slug', 'title', 'nmc', 'schedule' ,'qualification', 'facility_title','team', 'feature','color', 'image', 'header_image', 'image2', 'image3','offers', 'content', 'number_room', 'extra_bed', 'pac_icon','currency', 'discount', 'people_qnty', 'onep_price', 'twop_price', 'threep_price', 'meta_title', 'meta_keywords', 'meta_description', 'status', 'sortorder', 'added_date', 'type','offer_link');
	...
	public static function find_by_type($type=0){
		global $db;
		$sql = "SELECT * FROM ".self::$table_name." WHERE type='$type' LIMIT 1";
		$result_array = self::find_by_sql($sql);
		return !empty($result_array) ? array_shift($result_array) : false;
	}

	public static function find_position_by_type($type=0){
		global $db;
		$sql = "SELECT nmc FROM ".self::$table_name." WHERE type='$type' LIMIT 1";
		$res = $db->query($sql);
	...
	public static function getDoctor_limit($type=0,$limit=''){
		global $db;
	...
	public static function getTotalSub($type=''){
		...
		static function find_all_t(){
		global $db;
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE team=2  ORDER BY sortorder DESC");
	}

	public static function get_all_seldoc($actid=0,$selid=''){
		global $db;		
		$sql = "SELECT id,title FROM ".self::$table_name." WHERE type ='$actid' ORDER BY sortorder DESC";
		$record = self::find_by_sql($sql);
		$result='';
		if($record){
				$result.='<option value="all">Choose doctor</option>';
	...
	public static function find_all(){
		...
		static function find_all_doc(){
		global $db;
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE team=1  ORDER BY sortorder DESC");
	}
	...
	public static function find_by_sql($sql=""){
		...
		// view doctor Front.
		static function getDoctorbytype($txt,$type=0){
			global $db;
			$cond = !empty($type)?' AND type='.$type :'';
			$cond1 = !empty($txt)?' AND title like \'%'.$txt.'%\'':'';
			$sql = "SELECT * FROM ".self::$table_name." WHERE status=1 $cond1  $cond ORDER BY sortorder DESC ";
			return self::find_by_sql($sql);
		}
