            ...
            foreach ($menusubRec as $menusubRow):
                ...
                if ($menusub2Rec):
                    ...
                    foreach ($menusub2Rec as $menusub2Row):
                        ...
                        $drop3 = !empty($menusub3Rec) ? 'class="has_child"' : '';
                        $result .= '<li id="' . $menusub2Row->id . '" ' . $drop3 . '>';
...
if ($menuRec):
    ...
    foreach ($menuRec as $menuRow):
        ...
        /* Second Level Menu */
        if ($menusubRec):
            $result .= '<ul class="mdropdown_child">';
            foreach ($menusubRec as $menusubRow):
                $menusub2Rec = Menu::getMenuByParent($menusubRow->id, 1);
                $chkparent2 = (!empty($menusub2Rec)) ? 1 : 0;
                $drop2 = !empty($menusub2Rec) ? 'mdropdown' : '';
                $result .= '<li>';
                $result .= getMenuList($menusubRow->name, $menusubRow->linksrc, $menusubRow->linktype, $drop2, $chkparent2);
                /* Third Level Menu */
                if ($menusub2Rec):
                    $result .= '<ul class="mdropdown_child">';
                    foreach ($menusub2Rec as $menusub2Row):
...
$jVars['module:quick-links'] = $resfooter;
$resfooter = '';

$FmenuRec = Menu::getMenuByParent(0, 3);
if ($FmenuRec):
    $resfooter .= '<h3>Upcoming Event</h3><ul>';
...
$jVars['module:upcoming-events'] = $resfooter;
$resfooter = '';
